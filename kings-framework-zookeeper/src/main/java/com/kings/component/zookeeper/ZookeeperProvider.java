...
class ZookeeperProvider extends AbstractZookeeper<Serializable> implements OctopusZookeeper {
     ...
     */
    @Override
    public Serializable get(String s, ZookeeperSerializer serializer) throws OctopusZookeeperException {
        try {
    ...
    public Collection<ZookeeperTransactionResponse> inTransaction(
            Consumer<ZookeeperTransaction<String, Serializable>> action) throws OctopusZookeeperTransactionException {
        //先开启事物
    ...
    public OctopusZookeeper inAsync(Consumer<ZookeeperAsync<String, Serializable>> action,
        ...
        try (final ZookeeperAsync<String, Serializable> asyncOperator = this.async.openAsync(callback, listener)) {
    ...
    public OctopusZookeeper registerPathWatcher(final String path,
                                                final ZookeeperPathWatcher<String,
                                                        Serializable> pathListener, ZookeeperSerializer serializer)
            ...
            node.getListenable().addListener(() -> {
                ...
                if (node.getCurrentData() != null) {
                    ...
                    pathListener.nodeChanged(currentData.getPath(),
                            this.deserialize(serializer == null ? this.serializer : serializer, currentData.getData()));
                } else {
                    //避免不调用和空指针
    ...
    public OctopusZookeeper registerPathChildrenWatcher(
        ...
        try {
            ...
            if (this.nonexistent(path)) {
                throw new OctopusZookeeperException(NO_PATH);
            }
            childrenCache = new PathChildrenCache(this.curatorFramework, path0(path), true, false, executorService);
            childrenCache.getListenable().addListener((c, e) -> {
                ...
                final Serializable d =
                        this.deserialize(serializer == null ? this.serializer : serializer, data.getData());
                switch (type) {
                    case CHILD_ADDED:
                        //添加了子节点
    ...
    public OctopusZookeeper registerPathAndChildrenWatcher(
            ...
            ZookeeperPathAndChildrenWatcher<String, Serializable> listener, ZookeeperSerializer serializer)
            throws OctopusZookeeperWatcherException {
        ...
        try {
            ...
            treeCache.getListenable().addListener((client, event) -> {
                ...
                final String p = data.getPath();
                Serializable d = this.deserialize(serializer == null ? this.serializer : serializer, data.getData());
    ...
    public ZookeeperWriter<String, Serializable> create(
            String s, Serializable s2, NodeMode mode, boolean recurse, ZookeeperSerializer serializer)
